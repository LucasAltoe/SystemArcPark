...
def validar_usuario_logado(request: Request) -> Usuario | None:
    try:
        ...
        if token.strip() == "":
            return None 
        usuario = EstacionamentoRepo.getUserByToken(token)
        return usuario
    except KeyError:
        return None

def validar_usuario_cliente_logado(request: Request) -> Usuario | None:
    try:
        token = request.cookies["auth_token"]
        if token.strip() == "":
            return None
        usuario_cliente = ClienteRepo.getUserByToken(token)
